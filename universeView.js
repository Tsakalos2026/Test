(() => {
  ...
  function drawAtom(state, timeMs) {
    ...
    // Only show atom once you have electrons (you can change this condition)
    if ((state.electrons ?? 0) <= 0) return;
    ...
    // electrons
    const n = Math.min(8, Math.max(0, state.electrons | 0)); // cap for visuals
    const electrons = [];
  ...
  // Expose a single function your game can call:
  window.renderUniverse = (state) => {
    // store latest state reference; animation loop will read it
    window.__universeState = state;
  };

  function loop(t) {
    drawAtom(window.__universeState || { electrons: 0 }, t);
